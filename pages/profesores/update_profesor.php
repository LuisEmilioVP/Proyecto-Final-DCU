<?php
  ...
  if(isset($_POST['update'])){
    ...
    $phone = filter_var($phone, FILTER_SANITIZE_STRING);
     ...
     $update_admin = $connect->prepare("UPDATE `profesor` SET Nombre = ?, Apellido = ?, Genero = ?, Correo = ?, Telefono = ?, Edad = ?, Direccion = ?, Especialidad = ? WHERE IdProfesor = ?");
     $update_admin->execute([$name, $lastname, $gender, $email, $phone, $age, $direction, $specialty, $id]);
...
<body>
  ...
  <section class="update-product">
    <h1 class="heading">Actualizar Profesor</h1>
    <?php 
    $update_id = $_GET['update'];
    $show_admin = $connect->prepare("SELECT * FROM `profesor` WHERE IdProfesor = ?");
    ...
    <form action="" method="post" enctype="multipart/form-data">
      ...
      <input type="hidden" name="id" value="<?= $fetch_admin['IdProfesor']; ?>">
